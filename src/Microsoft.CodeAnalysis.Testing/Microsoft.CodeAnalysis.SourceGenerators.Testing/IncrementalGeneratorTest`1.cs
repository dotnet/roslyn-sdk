...
namespace Microsoft.CodeAnalysis.Testing
{
        ...
        where TVerifier : IVerifier, new()
    {
        ...
        /// <summary>
        /// Returns the incremental source generators being tested - to be implemented in non-abstract class.
        /// </summary>
        /// <returns>The <see cref="IIncrementalGenerator"/> to be used.</returns>
        protected abstract (IEnumerable<ISourceGenerator> sourceGenerators, IEnumerable<IIncrementalGenerator> incrementalGenerators) GetGenerators();

        protected abstract GeneratorDriver CreateGeneratorDriver(Project project, ImmutableArray<ISourceGenerator> sourceGenerators, ImmutableArray<IIncrementalGenerator> incrementalGenerators);
        ...
        protected override async Task RunImplAsync(CancellationToken cancellationToken)
        {
            ...
            var diagnostics = await VerifyGeneratorAsync(testState, Verify, cancellationToken).ConfigureAwait(false);
        ...
        {
            var (sourceGenerators, incrementalGenerators) = GetGenerators();
            var (finalProject, diagnostics) = await ApplyGeneratorsAsync(sourceGenerators.ToImmutableArray(), incrementalGenerators.ToImmutableArray(), project, verifier, cancellationToken).ConfigureAwait(false);
            return (await finalProject.GetCompilationAsync(cancellationToken).ConfigureAwait(false))!;
        }

        /// <summary>
        /// Called to test a C# incremental source generator when applied on the input source as a string.
        /// </summary>
        /// <param name="testState">The effective input test state.</param>
        /// <param name="verifier">The verifier to use for test assertions.</param>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> that the task will observe.</param>
        /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
        protected async Task<ImmutableArray<Diagnostic>> VerifyGeneratorAsync(SolutionState testState, IVerifier verifier, CancellationToken cancellationToken)
        {
            var (sourceGenerators, incrementalGenerators) = GetGenerators();
            return await VerifyGeneratorAsync(Language, sourceGenerators.ToImmutableArray(), incrementalGenerators.ToImmutableArray(), testState, ApplyGeneratorsAsync, verifier.PushContext("Source generator application"), cancellationToken);
        }

        private async Task<ImmutableArray<Diagnostic>> VerifyGeneratorAsync(
            string language,
            ...
            Func<ImmutableArray<ISourceGenerator>, ImmutableArray<IIncrementalGenerator>, Project, IVerifier, CancellationToken, Task<(Project project, ImmutableArray<Diagnostic> diagnostics)>> getFixedProject,
            IVerifier verifier,
            CancellationToken cancellationToken)
        {
            ...
            (project, diagnostics) = await getFixedProject(sourceGenerators, incrementalGenerators, project, verifier, cancellationToken).ConfigureAwait(false);
        ...
        private async Task<(Project project, ImmutableArray<Diagnostic> diagnostics)> ApplyGeneratorsAsync(ImmutableArray<ISourceGenerator> sourceGenerators, ImmutableArray<IIncrementalGenerator> incrementalGenerators, Project project, IVerifier verifier, CancellationToken cancellationToken)
        {
            var compilation = await project.GetCompilationAsync(cancellationToken).ConfigureAwait(false);
            verifier.True(compilation is { });

            var driver = CreateGeneratorDriver(project, sourceGenerators, incrementalGenerators).RunGenerators(compilation, cancellationToken);
