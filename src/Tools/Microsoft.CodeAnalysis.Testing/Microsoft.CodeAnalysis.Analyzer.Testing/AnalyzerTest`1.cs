...
namespace Microsoft.CodeAnalysis.Testing
{
        ...
        where TVerifier : IVerifier, new()
    {
        ...
        protected async Task VerifyDiagnosticsAsync((string filename, SourceText content)[] sources, (string filename, SourceText content)[] additionalFiles, DiagnosticResult[] expected, CancellationToken cancellationToken)
        {
            var analyzers = GetDiagnosticAnalyzers().ToImmutableArray();
            VerifyDiagnosticResults(await GetSortedDiagnosticsAsync(sources, additionalFiles, analyzers, cancellationToken).ConfigureAwait(false), analyzers, expected);
        ...
        private Solution GetSolution((string filename, SourceText content)[] sources, (string filename, SourceText content)[] additionalFiles)
        {
            ...
            var project = CreateProject(sources, additionalFiles, Language);
        ...
        protected Project CreateProject((string filename, SourceText content)[] sources, (string filename, SourceText content)[] additionalFiles, string language)
        {
            ...
            var project = CreateProjectImpl(sources, additionalFiles, language);
